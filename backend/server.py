...
async def get_mappings(partner_uni: str):
    try:
        result = conn.execute(
            f"select * from mappings where partner_uni = ?", [partner_uni]
        ).fetchdf()
        result = result.fillna("")
        return result.to_dict("records")
    except Exception as e:
...
async def get_universities_group_by_first_letter():
    try:
        result = conn.execute(
            f"SELECT LIST(university_name ORDER BY university_name) as university_names, university_name[:1] as first_letter FROM partner_unis GROUP BY 2 ORDER BY 2"
        ).fetchdf()
        result = result.fillna("")
        return result.to_dict("records")
    except Exception as e:
...
async def get_university(university_name: str):
    try:
        result = conn.execute(
            f"select * from partner_unis where university_name = ?", [university_name]
        ).fetchdf()
        result = result.fillna("")
        return result.to_dict("records")
    except Exception as e:
...
async def get_mappings_by_country(country: str):
    try:
        result = conn.execute(
            f"select * from mappings where partner_uni in (select university_name from partner_unis where university_country = ? )",
            [country],
        ).fetchdf()
        result = result.fillna("")
...
async def get_universities_by_country(country: str):
    try:
        result = conn.execute(
            f"select university_name from partner_unis where university_country = ?",
            [country],
        ).fetchdf()
        result = result.fillna("")
        return result.to_dict("records")
...
async def get_popular_countries():
    try:
        result = conn.execute(
            f"select university_country from (select university_country, count(*) as count from partner_unis group by university_country order by count desc limit 10)"
        ).fetchdf()
        result = result.fillna("")
        return result.to_dict("records")
    except Exception as e:
...
async def get_all_countries():
    try:
        result = conn.execute(
            f"select distinct university_country from partner_unis"
        ).fetchdf()
        result = result.fillna("")
        return result.to_dict("records")
    except Exception as e:
